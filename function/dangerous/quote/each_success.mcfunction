...
execute if score $ssm ssm_count matches 0 store success score $ssm ssm_success run data modify storage ssm: w.data.item_copy set value "\""
execute if score $ssm ssm_count matches 0 if score $ssm ssm_success matches 0 run data modify storage ssm: w.data.replaced_list append value "\\\""
